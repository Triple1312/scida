...
class DataFrame {
  ...
  DataFrame.fromLL(List<List<dynamic>> data, {bool inferTypes = false}) {
    int i = 7;
  }

  DataFrame.from_csv(String path, {bool inferTypes = false, List<Type> columntypes = const [], bool removePadding = false, String sep=','}) {
  ...
  DataFrame.from_dict(Map<String, Map<int, dynamic>> data, {bool inferTypes = false}) {
  ...
  DataFrame.from_ndarray(NDarray ndarray ,{bool inferTypes = false}) {
    if (ndarray.depth > 2) {
  ...
  DataFrame({ // todo should really do something
    Object? data,
    List<dynamic>? index,
    List<dynamic>? dtypes,
  ...
  DataFrame transpose() {
    ...
    return DataFrame.fromLL(transposed);
  }
  ...
  DataFrame assign(List<DFColumn> cols) {
    DataFrame df = copy();
  ...
  dynamic dot({DataFrame? df, DFColumn? col, Series? ser}) { // todo make listType so Columns work too | returnTypes are weird maybe
    ...
    if (df != null) {
      return DataFrame.fromLL(mdot(as_type(int) as List<List<int>>, df.as_type(int) as List<List<int>>));
    }
  ...
  DataFrame copy() {
    return DataFrame.fromLL(asList());
  }
...
class _DFColumnIterator implements Iterator<DFColumn> {
  final DataFrame df; // are Pointers an idea ??
  int _index = -1;
