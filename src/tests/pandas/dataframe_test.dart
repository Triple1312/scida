...
void main() {
  group("Integer Dataframe Tests:", () {
    ...
    test("any", () {
      expect(df.any(), {'Column_1': true, 'Column_2': true, 'Column_3': true, 'Column_4': true, 'Column_5': true});
    });
    test("count", () {
  ...
  group("RandomIntegersMixed Column Tests:",() {
    var df = DataFrame.from_csv("src/tests/pandas/RandomIntegersMixed.csv", inferTypes: true);
    DFColumn col5 = df.getColumn("Column_5")!;
    DFColumn col2 = df.getColumn("Column_2")!;
    DFColumn col3 = df.getColumn("Column_3")!;
    DFColumn col1 = df.getColumn("Column_1")!;
    test("ColumnType Int", () {
    ...
    test("empty column values", () {
      expect(col1.asList(), [null, null, null, null, null, null]);
    });
    test("dynamic values", (){
      expect(col3.asList(), ["PEkyw", 22.283637255811087, null, "ZIaOY", 25, null]);
    });
    DFColumn col3_e = col3.copy()..fillna(69);
    test("copy + fillna", () {
      expect(col3_e.asList(), ["PEkyw", 22.283637255811087, 69, "ZIaOY", 25, 69]);
    });
  });
  group("RandomIntegersMixed Dataframe Tests", (){
    var df = DataFrame.from_csv("src/tests/pandas/RandomIntegersMixed.csv", inferTypes: true);
    test("any", () {
      expect(df.any(), {'Column_1': false, 'Column_2': true, 'Column_3': true, 'Column_4': true, 'Column_5': true});
    });
    test("all", () {
